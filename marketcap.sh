...
curl -s -o marketcap.json "${API_URL}${PARAMS}"

# Debug: Print the first item's structure
echo "First item structure:"
cat marketcap.json | jq '.[0]'

# Clear the output file if it exists
> "$OUTPUT_FILE"

# Parse the JSON response and extract the market cap for each cryptocurrency
for row in $(cat marketcap.json | jq -r '.[] | @base64'); do
    _jq() {
    ...
    echo "Writing to file: ${name}: ${cap}"
    
    echo "${name}: ${cap}" >> "$OUTPUT_FILE"
done
